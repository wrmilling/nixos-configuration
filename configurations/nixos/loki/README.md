...
loki is an Lenovo ThinkCentre M715q with 16GB ram and a 250GB drive. 

## Installation

### Disk Setup

Creates 2 partitions on the drive, one for EFI Boot and one for the luks encrypted root.
...
```
$ mkdir -p hosts/loki
$ mv ../configuration.nix hosts/loki/default.nix
$ mv ../hardware-configuration.nix hosts/loki/hardware.nix
$ vim hosts/loki/default.nix
...
# Import Lanzaboote and generate new signed EFI blobs, already done for loki
$ sudo sh -c "cd /etc/nixos && git pull && nixos-rebuild switch --flake ."
