...
new_version=$(gh api repos/YoanWai/agent-manager/releases/latest --jq '.tag_name | ltrimstr("v")')

if [[ "$new_version" == "$current_version" ]]; then
  echo "agent-manager already up to date at $current_version"
  exit 0
fi

echo "Updating agent-manager to version: ${new_version}" >&2
tag="v${new_version}"
...
src_json="$(
  nix store prefetch-file \
    --json \
    --unpack \
    "https://github.com/YoanWai/agent-manager/archive/refs/tags/${tag}.tar.gz"
)"
...
echo "Updated agent-manager $current_version -> $new_version"
