...
new_version=$(curl --silent -fsSLI -o /dev/null -w '%{url_effective}' "https://github.com/joshuakto/fit/releases/latest" | sed -n 's#.*/releases/tag/##p')

if [[ "$new_version" == "$current_version" ]]; then
  echo "obsidian-fit already up to date at $current_version"
  exit 0
fi
...
while read -r key asset; do
  [ -z "$key" ] && continue
  url="https://github.com/joshuakto/fit/releases/download/${new_version}/${asset}"
...
echo "Updated obsidian-fit $current_version -> $new_version"
