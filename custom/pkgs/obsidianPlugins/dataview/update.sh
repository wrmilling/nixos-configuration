...
new_version=$(curl --silent -fsSLI -o /dev/null -w '%{url_effective}' "https://github.com/blacksmithgu/obsidian-dataview/releases/latest" | sed -n 's#.*/releases/tag/##p')

if [[ "$new_version" == "$current_version" ]]; then
  echo "obsidian-dataview already up to date at $current_version"
  exit 0
fi
...
while read -r key asset; do
  [ -z "$key" ] && continue
  url="https://github.com/blacksmithgu/obsidian-dataview/releases/download/${new_version}/${asset}"
...
echo "Updated obsidian-dataview $current_version -> $new_version"
