...
new_version=$(gh api repos/StepanKropachev/obsidian-pm/releases/latest --jq .tag_name)

if [[ "$new_version" == "$current_version" ]]; then
  echo "obsidian-project-manager already up to date at $current_version"
  exit 0
fi
...
while read -r key asset; do
  [ -z "$key" ] && continue
  url="https://github.com/StepanKropachev/obsidian-pm/releases/download/${new_version}/${asset}"
...
echo "Updated obsidian-project-manager $current_version -> $new_version"
