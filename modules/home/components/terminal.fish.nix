...
in
{
  options.modules.home.terminal.fish = {
    enable = lib.mkEnableOption "fish packages / settings";
  };
  ...
  config = lib.mkIf cfg.enable {
    programs.fish = {
      ...
      shellAbbrs = rec {
        ...
        # nvim
        vim = lib.mkIf hasNeovim "nvim";
        vi = vim;
        v = vim;

        # exa
        ls = lib.mkIf hasExa "exa";
        ll = lib.mkIf hasExa "exa -laag --git --icons --sort=type";
        llrt = lib.mkIf hasExa "exa -laag --git --icons -snew";
        l = lib.mkIf hasExa "exa -l --git --icons --sort=type";
        ...
        nhs = "cd ~/.nixos-configuration && git pull && nh os switch . && nh home switch . && cd -";
        nhb = "cd ~/.nixos-configuration && git pull && nh os boot . && echo 'Manually switch Home Manager next boot' && cd -";
      ...
      functions = {
        ...
        # hut patchset apply and patchset update applied
        hpaa = {
          description = "Applies a patchset from a sourcehut list and updates the status to APPLIED";
          body = ''
            hut lists patchset apply $argv && hut lists patchset update --status APPLIED $argv
          '';
        };
