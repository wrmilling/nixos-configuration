{
  ...
  hasDocker = config.virtualisation.docker.enable || config.virtualisation.docker.rootless.enable;
...
in {
  ...
  options.services.forgejo-actions-runner = with types; {
    package = mkPackageOptionMD pkgs "forgejo-actions-runner" {};
  ...
  config = mkIf (cfg.instances != {}) {
    assertions = [
      ...
      }
      {
        ...
        message = "Label configuration on forgejo-actions-runner instance requires either docker or podman.";
      }
    ];
          ...
          serviceConfig =
            {
              ...
              ExecStartPre = [
                (pkgs.writeShellScript "forgejo-register-runner-${name}" ''
