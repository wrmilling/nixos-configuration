...
let
  ...
  cfg = config.services.renovate-custom;
...
in
{
  ...
  options.services.renovate-custom = {
    ...
    settings = mkOption {
      ...
      description = ''
        ...
        If you want to pass secrets to renovate, please use {option}`services.renovate-custom.credentials` for that.
      '';
    };
  };

  config = mkIf cfg.enable {
    services.renovate-custom.settings = {
      cacheDir = "/var/cache/renovate";
      baseDir = "/var/lib/renovate";
    };

    systemd.services.renovate-custom = {
